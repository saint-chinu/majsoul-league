...
from analyze_ryanmen_riichi_wait_patterns import msg_tiles, remove_one
...
DEFAULT_OUTPUT = "nagaremono_table_event_log_with_hands.csv"
...
def tile_sort_key(tile: str) -> tuple[int, int, str]:
    ...
    if len(tile) != 2 or not tile[0].isdigit():
        return (9, 99, tile)
    return (SUIT_ORDER.get(tile[1], 9), int(tile[0]), tile)


def hand_text(hand: Counter[str]) -> str:
    tiles = []
    for tile, count in hand.items():
        tiles.extend([tile] * count)
    return " ".join(sorted(tiles, key=tile_sort_key))


def all_hands_json(hands: dict[int, Counter[str]]) -> str:
    return json.dumps({str(seat): hand_text(hands[seat]) for seat in sorted(hands)}, ensure_ascii=False, separators=(",", ":"))


def add_tile(hand: Counter[str], tile: str) -> None:
    tile = normalize_tile(tile)
    if tile:
        hand[tile] += 1
...
def hule_point_text(hule) -> str:
    parts = []
    for name in ["point_sum", "point_rong", "point_zimo_qin", "point_zimo_xian", "dadian"]:
...
def export_event_log(target_player: str, output_name: str) -> None:
    ...
    for source in source_rows:
        ...
        hands: dict[int, Counter[str]] = {0: Counter(), 1: Counter(), 2: Counter()}
        round_no = 0
        chang = ""
        ju = ""
        honba = ""
        ...
        def make_row(event_type: str, record_type: str, seat="", hand_before="") -> dict[str, str]:
            nonlocal event_no_in_game, event_no_in_round
            event_no_in_game += 1
            event_no_in_round += 1
            player = seat_to_name.get(seat, f"seat{seat}") if seat != "" else ""
            return {
                ...
                "seat": seat,
                "player": player,
                "is_target_player": 1 if seat in target_seats else (0 if seat != "" else ""),
                "tile": "",
                "normalized_tile": "",
                "is_red_five": "",
                "is_tsumogiri": "",
                ...
                "hand_after": hand_text(hands[seat]) if seat != "" else "",
                "seat0_hand_after": hand_text(hands[0]),
                "seat1_hand_after": hand_text(hands[1]),
                "seat2_hand_after": hand_text(hands[2]),
                "all_hands_after": all_hands_json(hands),
        ...
        for wrapper in wrappers:
            ...
            if record_name == ".lq.RecordNewRound":
                round_no += 1
                hands = {
                    0: Counter(normalize_tile(tile) for tile in getattr(msg, "tiles0", [])),
                    1: Counter(normalize_tile(tile) for tile in getattr(msg, "tiles1", [])),
                    2: Counter(normalize_tile(tile) for tile in getattr(msg, "tiles2", [])),
                }
                ...
                event_no_in_round = 0
                row = make_row("局開始", record_name)
                row["scores_after"] = json_value(round_start_scores)
                exported.append(row)
                continue

            if record_name == ".lq.RecordDealTile":
                seat = get_int(msg, "seat")
                tile = normalize_tile(getattr(msg, "tile", ""))
                if seat == "" or not tile:
                    continue
                before = hand_text(hands[seat])
                add_tile(hands[seat], tile)
                row = make_row("ツモ", record_name, seat, before)
                row["tile"] = getattr(msg, "tile", "")
                row["normalized_tile"] = tile
                row["is_red_five"] = 1 if str(getattr(msg, "tile", "")).startswith("0") else 0
                exported.append(row)
                continue
            ...
            if record_name == ".lq.RecordDiscardTile":
                ...
                if seat == "" or not normalized:
                    continue
                before = hand_text(hands[seat])
                remove_one(hands[seat], normalized)
                row = make_row("リーチ" if is_riichi_discard(msg) else "打牌", record_name, seat, before)
                row["tile"] = tile
                row["normalized_tile"] = normalized
            ...
            if record_name == ".lq.RecordBaBei":
                ...
                if seat == "":
                    continue
                before = hand_text(hands[seat])
                remove_one(hands[seat], "4z")
                row = make_row("北抜き", record_name, seat, before)
                row["tile"] = "4z"
            ...
            if record_name in [".lq.RecordChiPengGang", ".lq.RecordAnGangAddGang"]:
                ...
                before = hand_text(hands[seat])
                for tile in tiles:
                    remove_one(hands[seat], tile)
                row = make_row("鳴き/槓", record_name, seat, before)
            ...
            if record_name == ".lq.RecordHule":
                ...
                for hule in getattr(msg, "hules", []):
                    ...
                    row = make_row("和了", record_name, winner_seat)
