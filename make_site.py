...
LABELS = {
    ...
    "nanba_comeback_rate": "南場まくり率",
    "nanba_overtaken_rate": "南場まくられ率",
...
METRIC_DESCRIPTIONS = {
    ...
    "nanba_comeback_rate": "南1局開始時に1位以外だった半荘のうち、オーラス終了時の最終順位を上げて終えた割合。",
    "nanba_overtaken_rate": "南1局開始時に3位以外だった半荘のうち、オーラス終了時の最終順位を下げて終えた割合。",
...
PLAYER_RANK_METRICS = [
    ...
    ("nanba_comeback_rate", True, "高い方が上位"),
    ("nanba_overtaken_rate", False, "低い方が上位"),
...
# 新リーグは、機械的な順位比較ではなく直近135半荘の実数値を読んだ所見を掲載する。
MANUAL_NEW_LEAGUE_ANALYSIS: dict[str, tuple[str, str, str]] = {
    "流れ者金融": (
        "新リーグでも主導権型。47半荘で獲得スコア+291.0、平均順位1.83は全員中トップで、和了率32.54%・先制テンパイ率38.58%・テンパイ維持率23.32%も首位級。副露率35.34%を使って速度を取りながら、トップ滞在率45.04%まで先行時間を伸ばしている。門前一本ではなく、点棒状況に応じて鳴きで局を短くできるため、相手に手を育てる余地を与えにくい。",
        "特に良いのは、速さと着順のつながり方。1位率40.43%、ラス回避率76.60%に対して放銃率は16.81%で、押している量の割に崩れが少ない。リーチ質は可率84.13%、鳴き質も+0.23で上位。先制した局を丁寧に取り切る一方、仕掛けても手役・形が足りる局を選べている。今季の強さは大物手の偏りというより、局収支の小さな優位を積み上げているところにある。",
        "課題は、首位時の守りへの切替をさらに明確にすること。トップキープ率27.27%は能力の割に伸びしろがあり、先制テンパイから押し返す局で着順期待と局収支を分け切れていない可能性がある。南場トップ目では、良形でも安手・残り枚数の薄い追っかけを一段抑え、2着との点差を保つ局消化へ寄せたい。攻撃を削るより、首位時だけ押しの条件を厳しくするのが一番効く。",
    ),
    "アリスkey": (
        "攻守の配分が最も整っている上昇型。55半荘で+140.8、平均順位1.98、和了率31.54%に対して放銃率14.10%はリーグ最上位の守備水準。副露率32.10%で局参加も多く、先制テンパイ率34.51%まで取れている。リーチを乱発せず、門前と副露を使い分けて『あがれる局だけを取りに行く』姿勢が数字に出ている。",
        "リーチ面の成熟も目立つ。リーチ質スコア3.37、リーチ可率88.81%は全員で最良級で、待ち・残り枚数・打点を揃えた局を選べている。トップキープ率45.71%も突出しており、一度前に出た後に無理な押し合いを避け、着順を守る判断が機能している。ラス回避率70.91%と放銃の軽さの組み合わせは、安定して上位に残るためのかなり強い土台。",
        "伸ばすなら、トップへ届く局の最後の一押し。1位率30.91%は守備力・トップキープ率と比べると低く、2着や僅差トップを安全にまとめる局がやや多い。親番、2着目、ドラが使える良形一向聴では、守備牌を抱えすぎず打点ルートを残したい。鳴き質-0.24も、早いが安い仕掛けが少し混ざるサインなので、トップを取りに行く鳴きだけ打点条件を上げると収支がさらに伸びる。",
    ),
    "葡萄海ぶどう": (
        "打点と局収支の釣り合いが改善してきた攻撃寄りのバランス型。58半荘で+118.7、平均順位2.00、和了率29.75%、立直率29.93%。平均和了点11,653.8点は上位で、門前リーチの収入は明確な武器。放銃率13.03%も低く、従来の『高打点を追って沈む』形からはかなり離れている。",
        "特に評価したいのは、攻撃量を保ったまま守備の精度を上げた点。リーチ率は全体上位なのに放銃平均9,097点、ラス回避率67.24%で、勝負をした局でも致命傷を避けられている。トップ滞在率43.66%、トップキープ率32.43%も悪くなく、前に出た半荘をまとめる力は以前より安定している。リーチ質3.02は十分高く、曲げる局の選別自体は大きな問題ではない。",
        "次の課題は先制率と鳴きの価値。先制テンパイ率32.22%、テンパイ維持率19.63%は中位で、重い手へ向かう局に速度負けが混ざる。鳴き質-0.16も、手役を見込めない仕掛けが少数あることを示す。親番やラス目で高打点を狙う個性は残しつつ、平場ではタンヤオ・役牌・良形の速度ルートを一つ増やしたい。和了率を1ポイント上げるだけで、今の守備力なら順位が一段上がる。",
    ),
    "ぬちきち": (
        "少数試合ながら攻撃性能がはっきり出ている。23半荘で+90.2、平均順位1.91、1位率43.48%、和了率33.94%はいずれも強い。先制テンパイ率35.32%と立直率28.44%で門前の攻撃を主軸にしつつ、平均和了点10,545.9点を確保している。小さく和了回数だけを増やすのでなく、着順を動かせる手で前へ出ているタイプ。",
        "リーチ質85.48%、鳴き質+0.23は内容面でも良い。良形・必要打点を満たす局を選んでおり、1位率と和了率の高さが偶然のツモ偏重ではないことを支える。放銃率15.60%は少し高めでも、放銃平均9,714点に収まっており、勝負手での失点が中心。現状は『押して勝つ』麻雀として筋が通っている。",
        "ただし23半荘では分散がまだ大きい。トップキープ率23.81%、ラス回避率65.22%は、トップを取る力に比べるとやや物足りない。先行後は安い追っかけや終盤の無理押しを減らし、ラス目からの高打点勝負とトップ目の局消化を明確に分けたい。今の攻撃の鋭さを残しながら、前に出た半荘を2着以上で閉じる率を上げられれば、成績はかなり安定する。",
    ),
    "USAみん": (
        "17半荘ではあるが、トップ先行型の片鱗が濃い。平均順位1.94、1位率41.18%、トップ滞在率50.31%は非常に高く、一度流れを取った半荘で場を支配する時間が長い。和了率27.67%は突出しないが、平均和了点11,525点が高く、少ない和了を着順に直結させられている。",
        "リーチ質スコア3.28、可率86.96%は優秀で、門前で押す局の選別はかなり丁寧。副露率30.19%もあり、手牌が遅い局は鳴きで参加できている。先制テンパイ率28.93%に対しテンパイ維持率22.26%が高いのは、押し返しやダマを含めたテンパイ後の粘りが持ち味であることを示す。",
        "一方で放銃率16.35%とラス回避率64.71%は、トップ時の強さに比べて下振れ側が残る。先制できない局でテンパイまで粘る価値と、相手の高打点気配に対する撤退を分けたい。試合数が増えた時に1位率だけでなくラス回避率を70%近くへ寄せられるかが、今の好成績を実力として定着させる鍵になる。",
    ),
    "ルム@魚": (
        "守備を軸に着順をまとめる安定型。33半荘で平均順位1.91、ラス回避率78.79%、放銃率11.11%は全員中でも際立つ。リードを作るより、相手の加点を止めて最終局面で着順を拾う力があり、トップキープ率22.73%が低くても平均順位を保てている。平均和了点11,952.9点も高く、和了回数は少なくとも必要な局では収入を作れる。",
        "守備型でありながらリーチの内容も良い。リーチ質3.12、可率85.53%は高く、危険な曲げを避けながら高打点の門前手を拾えている。放銃平均9,936点はやや高いが、絶対的な放銃回数が少ないため半荘全体への傷は小さい。副露率22.22%が低いぶん、無理な仕掛けで手を崩さないのも今の数字には合っている。",
        "課題は局参加量。和了率25.44%、先制テンパイ率30.41%、テンパイ維持率18.85%はいずれも下側で、守備の良さが『何も起こせない局の多さ』にもつながっている。安全牌を持つ余裕があるタイプだからこそ、親番・ドラ対子・良形一向聴ではもう一枚効率に寄せてよい。放銃率を13%程度まで許容しても、和了率を2ポイント上げられれば総合順位はさらに強くなる。",
    ),
    "ひなんじょ": (
        "門前リーチ主導の高打点型。66半荘で立直率29.40%、和了率27.67%、平均和了点11,404.5点と、テンパイをリーチ収入に変える設計が明確。副露率27.52%もあるため門前一辺倒ではなく、先制テンパイ率30.66%を支える速度の選択肢は持っている。ただし獲得スコア-316.5、平均順位2.14で、収入の割に着順化が追いついていない。",
        "リーチの選別は良い。質スコア3.17、可率80.75%で、数を打ちながら十分な待ちを確保できている。トップキープ率26.32%は平均的でも、トップ滞在率35.69%に対して極端に落としてはいない。放銃率17.30%は重めだが、平均放銃8,925点は軽く、危険牌を掴んだ時も特大失点を抑える技術はある。",
        "改善は『攻撃の入口』より『押しの終点』。和了率が伸びないまま放銃率が高いので、良形化を待つ一向聴や、安手・残り枚数の少ないテンパイで相手のリーチへ押し返す局を絞りたい。副露後の質-0.05は大きな問題ではないが、門前の高打点ルートを残すべき局まで速度鳴きしていないかは見直しどころ。非テンパイ放銃を減らせれば、今の高打点力だけでスコアはすぐ上向く。",
    ),
    "鯛ofカルピス": (
        "局参加は多いが、失点管理が課題になっている攻撃型。60半荘で先制テンパイ率34.30%、テンパイ維持率22.32%、副露率30.58%と手を進める力は高い。和了率29.13%も悪くない一方、放銃率20.23%は明確に高く、獲得スコア-411.1、平均順位2.13へ直結している。手を止めずに戦う長所が、相手の高打点局でも押し切る短所へ反転している。",
        "それでも攻撃の中身には武器がある。リーチ質2.84、平均和了点10,958.3点で、曲げる局の質は中上位。トップキープ率38.24%も高く、前に出た後に守るだけでなく、追加加点で逃げ切る力がある。問題は攻撃を始める判断ではなく、他家のリーチ・二副露・ドラ周辺が揃った後にどこまで同じ強度で押すかだ。",
        "改善の優先順位ははっきりしている。まず非テンパイでの終盤押し、次に安手・愚形テンパイからの追っかけ、最後に鳴き手への危険牌処理。この三つで放銃率を20%から17%台へ落とせれば、和了率を下げずに平均順位が大きく動く。攻撃型の持ち味を消す必要はなく、相手の打点が見えた局だけ一段引く『例外ルール』を作るのがよい。",
    ),
    "29ちゃん": (
        "守備精度が際立つ待機・高打点型。46半荘で放銃率12.02%は最上位、平均放銃9,308点も軽く、ラス回避率67.39%を支えている。平均和了点12,576点は全員トップで、和了率24.64%と回数は少なくても、和了の一回が着順を大きく動かす。リーチ質2.78は十分で、待ちの悪い勝負を避ける姿勢も一貫している。",
        "今季はスコア+32.3、平均順位1.96までまとめられており、守備だけの麻雀ではない。トップキープ率51.85%は突出して高く、一度前に出た半荘を崩さない力はリーグ随一。放銃を抑えながら平均和了点を高く維持できるのは、危ない局を降り、勝負局では打点が足りるまで待つ判断が噛み合っているから。",
        "課題は明確に手数。先制テンパイ率30.96%、テンパイ維持率19.08%、和了率24.64%では、そもそも勝負局の回数が少ない。安全牌を抱えすぎる局、役牌対子やタンヤオが見える序盤での手組みを守備寄りにしすぎる局を少し減らしたい。放銃率は今のまま保てなくてもよく、13〜14%の範囲で先制テンパイを増やせれば、トップキープの強さをもっと頻繁に使える。",
    ),
}
        ...
        summary_rows.append(
            {
                ...
                "nanba_comeback_rate": percent(
                    player_stats.nanba_comebacks,
                    player_stats.nanba_comeback_chances,
                ),
                "nanba_overtaken_rate": percent(
                    player_stats.nanba_overtaken,
                    player_stats.nanba_overtaken_chances,
                ),
                "nanba_comeback_chances": str(player_stats.nanba_comeback_chances),
                "nanba_comebacks": str(player_stats.nanba_comebacks),
                "nanba_overtaken_chances": str(player_stats.nanba_overtaken_chances),
                "nanba_overtaken": str(player_stats.nanba_overtaken),
...
def player_analysis(
    ...
    manual_analysis: dict[str, tuple[str, str, str]] | None = None,
) -> tuple[str, str, str]:
    if manual_analysis and player in manual_analysis:
        return manual_analysis[player]
...
def render_player_panel(
    ...
    style, strength, advice = player_analysis(
        player,
        cumulative_rows,
        use_manual=use_manual_analysis,
...
def combine_player_rows(contexts: list[dict[str, object]]) -> list[dict[str, str]]:
    ...
    for player, rows in rows_by_player.items():
        ...
        nanba_comeback_chances = sum(parse_int(row.get("nanba_comeback_chances", 0)) for row in rows)
        nanba_comebacks = sum(parse_int(row.get("nanba_comebacks", 0)) for row in rows)
        nanba_overtaken_chances = sum(parse_int(row.get("nanba_overtaken_chances", 0)) for row in rows)
        nanba_overtaken = sum(parse_int(row.get("nanba_overtaken", 0)) for row in rows)
        ...
        combined_rows.append(
            {
                ...
                "nanba_comeback_rate": fmt_pct_count(nanba_comebacks, nanba_comeback_chances),
                "nanba_overtaken_rate": fmt_pct_count(nanba_overtaken, nanba_overtaken_chances),
                "nanba_comeback_chances": str(nanba_comeback_chances),
                "nanba_comebacks": str(nanba_comebacks),
                "nanba_overtaken_chances": str(nanba_overtaken_chances),
                "nanba_overtaken": str(nanba_overtaken),
...
def main() -> None:
    ...
    new_player_panels = "\n".join(
        render_player_panel(
            ...
            manual_analysis=MANUAL_NEW_LEAGUE_ANALYSIS,
        )
        for player in new_player_names
    )
