...
MANUAL_PLAYER_ANALYSIS: dict[str, tuple[str, str, str]] = {
    ...
    "ひなんじょ": (
        "門前圧力型。獲得スコア1,190.7、平均順位1.97、1位率35.64%で、プラス域の2番手にいる。立直率31.15%、和了率30.81%、先制テンパイ率33.04%、テンパイ維持率20.66%と、自分からテンパイを入れてリーチで場を縛る局が多い。トップ滞在率41.63%も高く、勝負どころでトップ目に立つ力は十分。一方でラス滞在率29.12%、ラス回避率67.23%なので、攻撃の裏目が沈み半荘へ直結する局もまだある。",
        "強みは、リーチを中心にした収入設計。リーチ率が高いのに和了率も30%を超え、平均和了点10,668.4点も高いので、ただ曲げているだけではなく、収入のあるテンパイにたどり着けている。副露の質スコア0.05はリーグ内でかなり良く、鳴きも雑な消化ではなく、テンパイや速度補助として機能している。リーチの質は平均スコア2.79、おすすめ寄り79.75%で、数を打ちながら最低限以上の質を保てている。",
        "改善点は、テンパイ前後の危険牌処理。放銃率16.88%、放銃平均9,411.8点はやや重く、特に良いテンパイへ向かう途中で失点が膨らんでいる可能性が高い。リーチ本数を減らすより、一向聴で親の濃い仕掛けに押す局、ドラが見えていない終盤、2着目でトップが遠い局の押しを削る方が効く。攻撃の軸ははっきりしているので、課題はアクセルではなくブレーキ位置。沈む半荘の放銃を1回減らせれば、トップ争い側へかなり寄る。",
    ),
    "鯛ofカルピス": (
        "自力決着型。獲得スコア719.9、平均順位1.99、和了率31.69%でしっかりプラス。先制テンパイ率36.28%、テンパイ維持率22.62%はどちらもリーグ最高水準で、誰よりも早くテンパイし、その状態を長く保てている。立直率24.15%は控えめなので、テンパイ量の多さはリーチ乱発ではなく、ダマ・副露・押し返しを含めた手牌進行の速さから来ている。トップ滞在率40.98%、トップキープ率31.04%で、先行した時の押し切り力もある。",
        "強みは、局を自力で終わらせに行く力。テンパイ維持率が高い人は、押し返されても完全撤退に回りにくく、流局まで相手にプレッシャーを残せる。リーチの質は平均スコア2.96、おすすめ寄り81.78%で、曲げる局もかなり選べている。和了率31.69%に対して平均和了点10,242.6点も十分で、速さだけに寄りすぎていない。守備を少し整えるだけで、トップ回数よりも2着・3着の失点圧縮でさらに伸びるタイプ。",
        "改善点は、防御面の温度管理。放銃率17.98%は最も重い部類で、テンパイ維持率の高さがそのまま押しすぎにもつながっている。自分が和了れる局が多いぶん、終盤の愚形・安手・残りツモが少ないテンパイでも粘りたくなるはず。親リーチ、ドラ周辺、明らかな高打点副露に対しては一段だけ冷やしたい。攻撃を弱める必要はないが、非テンパイや価値の低いテンパイでの放銃を減らせば、最高クラスの先制力がそのまま順位に変わる。",
    ),
    "アリスkey": (
        "速度参加型の改善勢。獲得スコアは-369.0まで戻しており、以前よりかなり見栄えが良くなった。平均順位1.99、ラス回避率68.84%、放銃率15.37%で、沈む半荘を投げるタイプではない。副露率32.55%、先制テンパイ率32.99%、テンパイ維持率20.44%で、重い配牌でも局へ関与する力は十分ある。後半シーズンにかけてスコアの戻しが見えており、序盤の負債を修正力で返しに行っているプレイヤー。",
        "強みは、局参加の柔軟さと守備のまとまり。鳴きで速度を作れるし、立直率27.09%もあるので門前勝負も捨てていない。テンパイ維持率20.44%は中位以上で、テンパイ後に簡単に受けへ回らない粘りもある。ラス滞在率28.33%は低めで、ずっと苦しい位置にいる時間はそこまで多くない。放銃率15.37%も悪くなく、参加量の割に崩れにくい。今は「負けない土台」はできつつあり、次の課題はトップ滞在率40.14%をトップキープ率26.03%へもっと変換すること。",
        "改善点は、トップ目と2着目での局面価値の整理。リーチの非推奨率22.28%はやや高めで、押す局の中にまだ余計な勝負が混ざっている。トップ目では安い愚形リーチや遠い仕掛けを少し抑え、2着目や親番ビハインドでは逆に良形・高打点ルートを逃さない。副露の質スコア-0.21もまだ改善余地があり、鳴いた後の打点・テンパイ形をもう少し選びたい。後半で強くなっている方向性は合っているので、勝っている半荘を失点でこぼさない精度が上がればプラス圏が見える。",
    ),
    "29ちゃん": (
        "守備基盤型。放銃率14.73%はリーグ最少、放銃平均9,014.9点も軽めで、三麻でこの失点管理はかなり価値がある。一方で獲得スコア-1,598.9、平均順位2.04、和了率26.98%、副露率27.42%、立直率23.03%、テンパイ維持率18.36%はいずれも控えめ。守れてはいるが、局を先に取りに行く量が足りず、守備力をスコアへ変換しきれていない。トップ滞在率37.46%は悪くないので、先行できた時は十分戦えている。",
        "強みは、半荘を壊さない我慢とリーチ選別の丁寧さ。低放銃の人は打点も低くなりがちだが、平均和了点10,420.4点は高く、守るだけのプレイヤーではない。リーチの質は平均スコア3.00、おすすめ寄り81.83%でリーグ最上位級。曲げるリーチ自体はかなり丁寧で、待ちや打点の選別はできている。だからこそ課題はリーチの質ではなく、先制テンパイ率30.76%、テンパイ維持率18.36%という到達回数と継続時間にある。",
    ...
    "葡萄海ぶどう": (
        "高打点志向のロマン砲型。平均和了点10,601.4点、リーチ率29.83%で、和了した時の破壊力は十分ある。一方で獲得スコア-4,723.0、平均順位2.14、和了率26.39%、ラス回避率59.11%、ラス滞在率33.51%がかなり苦しい。先制テンパイ率30.67%、テンパイ維持率19.18%も下位寄りで、高い手を作る意識に対して、テンパイまでの速度とテンパイ後の継続時間が少し足りていない。最大点は作れるが、最低点が深くなりやすいタイプ。",
        "強みは、打点ルートを見つけた時の押し切り力。七対子、ホンイツ、リーチ高打点のルートを選べるので、細かい和了で刻むより一度の大物手で着順を変える麻雀になっている。平均和了点はリーグ上位で、噛み合う半荘では一撃で着順を変えられる。放銃の質スコア-2.92は極端に悪いわけではなく、全てが無謀な押しというより、テンパイまで遅れて押し返す局が増え、その結果として苦しい放銃が混ざっている印象。個性そのものを消す必要はない。",
        "改善点は、勝負手ではない局の撤退速度と、重い手を追う基準。リーチの非推奨率25.61%が高めで、和了率が低いわりに放銃率17.24%が重い。打点種が薄い配牌、親の仕掛けが速い局、南場で2着を守れば十分な局では、満貫ルートを無理に追わず速度か守備に寄せたい。副露の質スコア-0.37も低く、鳴いた後の形・打点が足りない仕掛けが混ざっている。放銃率を2ポイント下げつつ、先制テンパイ率を少し上げるのが一番効く。",
    ),
}
