...
MANUAL_PLAYER_ANALYSIS: dict[str, tuple[str, str, str]] = {
    "流れ者金融": (
        "累計トップの支配型。獲得スコア4,780.3、平均順位1.85、1位率41.71%、ラス回避率73.70%と、シーズン10を終えても全体の軸はほぼ崩れていない。和了率32.33%、副露率35.29%、先制テンパイ率35.16%、テンパイ維持率21.56%で、門前だけでも副露だけでもなく、どの入口からでも局に触れる。トップ滞在率45.44%も高く、半荘のかなり長い時間をリード側で進めている。シーズン10で初めて単季マイナスと平均順位2.0割れを経験したが、累計では依然としてリーグで唯一の平均順位1.8台を保っている。",
        "強みは、攻撃量の多さと放銃の軽さが同居していること。これだけ局参加して放銃率15.11%・放銃平均8,803.3点はリーグ2番手の軽さで、放銃の質スコア-2.75はリーグで最も0に近い＝相対的に一番『刺さっても仕方ない放銃』で収まっている。リーチの質もおすすめ寄り84.61%、非推奨15.39%、平均スコア2.95で上位、副露の質スコア-0.11もリーグ最良級。トップキープ率36.36%もリーグ最高で、守るトップというより、リード後も局を終わらせに行くトップ。曲げる局・押す局の選別が数字に表れている。",
        "シーズン10のマイナスは『運が悪かっただけ』か——数字はおおむねそれを支持する。配牌の悪さも、リーチ後におっかけられて掴まされる放銃も大半は巡り合わせ側の要因で、実際にリーチの質・放銃の質・和了率はいずれも累計でリーグ上位を保ったままだ。プロセス指標が最上位クラスのまま単季だけ沈むのは、実力低下より分散のブレと読むのが自然で、来季は放っておいても戻りやすい。唯一の伸びしろは、配牌が重い局と着順ビハインドの局での撤退をあと一段だけ早めること。放銃の質は良くても平均8,803.3点は失っているので、悪配牌の半荘で無理に追いかけない選択を数局増やせば、良い季と悪い季の振れ幅そのものが縮み、累計首位はさらに固くなる。",
    ),
    "ひなんじょ": (
        "門前圧力型。獲得スコア1,036.8、平均順位1.97、1位率35.91%で、プラス域の2番手にいる。立直率31.38%、和了率30.99%、先制テンパイ率33.40%、テンパイ維持率20.85%と、自分からテンパイを入れてリーチで場を縛る局が多い。トップ滞在率41.59%も高く、勝負どころでトップ目に立つ力は十分。一方でラス滞在率29.37%、ラス回避率66.67%なので、攻撃の裏目が沈み半荘へ直結する局もまだある。",
        "強みは、リーチを中心にした収入設計。リーチ率が高いのに和了率も30%を超え、平均和了点10,611.5点も高いので、ただ曲げているだけではなく、収入のあるテンパイにたどり着けている。副露の質スコア0.04はリーグ内でかなり良く、鳴きも雑な消化ではなく、テンパイや速度補助として機能している。リーチの質は平均スコア2.74、おすすめ寄り79.65%で、数を打ちながら最低限以上の質を保てている。",
        "改善点は、テンパイ前後の危険牌処理。放銃率16.97%、放銃平均9,473.6点はやや重く、特に良いテンパイへ向かう途中で失点が膨らんでいる可能性が高い。リーチ本数を減らすより、一向聴で親の濃い仕掛けに押す局、ドラが見えていない終盤、2着目でトップが遠い局の押しを削る方が効く。攻撃の軸ははっきりしているので、課題はアクセルではなくブレーキ位置。沈む半荘の放銃を1回減らせれば、トップ争い側へかなり寄る。",
    ),
    "鯛ofカルピス": (
        "自力決着型。獲得スコア715.4、平均順位1.99、和了率31.78%でしっかりプラス。先制テンパイ率36.34%、テンパイ維持率22.65%はどちらもリーグ最高水準で、誰よりも早くテンパイし、その状態を長く保てている。立直率24.01%は控えめなので、テンパイ量の多さはリーチ乱発ではなく、ダマ・副露・押し返しを含めた手牌進行の速さから来ている。トップ滞在率41.28%、トップキープ率30.70%で、先行した時の押し切り力もある。",
        "強みは、局を自力で終わらせに行く力。テンパイ維持率が高い人は、押し返されても完全撤退に回りにくく、流局まで相手にプレッシャーを残せる。リーチの質は平均スコア2.97、おすすめ寄り81.80%で、曲げる局もかなり選べている。和了率31.78%に対して平均和了点10,169.0点も十分で、速さだけに寄りすぎていない。守備を少し整えるだけで、トップ回数よりも2着・3着の失点圧縮でさらに伸びるタイプ。",
        "改善点は、防御面の温度管理。放銃率18.07%は最も重い部類で、テンパイ維持率の高さがそのまま押しすぎにもつながっている。自分が和了れる局が多いぶん、終盤の愚形・安手・残りツモが少ないテンパイでも粘りたくなるはず。親リーチ、ドラ周辺、明らかな高打点副露に対しては一段だけ冷やしたい。攻撃を弱める必要はないが、非テンパイや価値の低いテンパイでの放銃を減らせば、最高クラスの先制力がそのまま順位に変わる。",
    ),
    "アリスkey": (
        "速度参加型の改善勢。獲得スコアは-772.4まで戻しており、以前よりかなり見栄えが良くなった。平均順位2.01、ラス回避率68.40%、放銃率15.34%で、沈む半荘を投げるタイプではない。副露率32.40%、先制テンパイ率32.73%、テンパイ維持率20.34%で、重い配牌でも局へ関与する力は十分ある。後半シーズンにかけてスコアの戻しが見えており、序盤の負債を修正力で返しに行っているプレイヤー。",
        "強みは、局参加の柔軟さと守備のまとまり。鳴きで速度を作れるし、立直率27.20%もあるので門前勝負も捨てていない。テンパイ維持率20.34%は中位以上で、テンパイ後に簡単に受けへ回らない粘りもある。ラス滞在率28.54%は低めで、ずっと苦しい位置にいる時間はそこまで多くない。放銃率15.34%も悪くなく、参加量の割に崩れにくい。今は「負けない土台」はできつつあり、次の課題はトップ滞在率39.99%をトップキープ率25.21%へもっと変換すること。",
        "改善点は、トップ目と2着目での局面価値の整理。リーチの非推奨率22.57%はやや高めで、押す局の中にまだ余計な勝負が混ざっている。トップ目では安い愚形リーチや遠い仕掛けを少し抑え、2着目や親番ビハインドでは逆に良形・高打点ルートを逃さない。副露の質スコア-0.21もまだ改善余地があり、鳴いた後の打点・テンパイ形をもう少し選びたい。後半で強くなっている方向性は合っているので、勝っている半荘を失点でこぼさない精度が上がればプラス圏が見える。",
    ),
    "29ちゃん": (
        "守備基盤型。放銃率14.73%はリーグ最少、放銃平均9,100.6点も軽めで、三麻でこの失点管理はかなり価値がある。一方で獲得スコア-1,441.8、平均順位2.04、和了率26.83%、副露率27.17%、立直率23.08%、テンパイ維持率18.36%はいずれも控えめ。守れてはいるが、局を先に取りに行く量が足りず、守備力をスコアへ変換しきれていない。トップ滞在率37.12%は悪くないので、先行できた時は十分戦えている。",
        "強みは、半荘を壊さない我慢とリーチ選別の丁寧さ。低放銃の人は打点も低くなりがちだが、平均和了点10,456.2点は高く、守るだけのプレイヤーではない。リーチの質は平均スコア3.02、おすすめ寄り81.97%でリーグ最上位級。曲げるリーチ自体はかなり丁寧で、待ちや打点の選別はできている。だからこそ課題はリーチの質ではなく、先制テンパイ率30.65%、テンパイ維持率18.36%という到達回数と継続時間にある。",
        "改善点は、親番と南場ビハインド時の攻撃量。序盤の良形・役牌対子・ドラ周辺はもう少し前向きに残してよい。特に親番で先制できる一向聴、ラス目南場で打点がある手、2着目からトップを狙える局では、安牌を抱えすぎず手牌効率を優先したい。守備力があるので、攻撃を少し増やしても大事故にはなりにくい。和了率を1から2ポイント上げられれば、低放銃という長所がそのまま累計スコアに変わる。",
    ),
    "葡萄海ぶどう": (
        "高打点志向のロマン砲型。平均和了点10,576.6点、リーチ率29.95%で、和了した時の破壊力は十分ある。一方で獲得スコア-4,255.0、平均順位2.14、和了率26.37%、ラス回避率59.53%、ラス滞在率33.42%がかなり苦しい。先制テンパイ率30.71%、テンパイ維持率19.30%も下位寄りで、高い手を作る意識に対して、テンパイまでの速度とテンパイ後の継続時間が少し足りていない。最大点は作れるが、最低点が深くなりやすいタイプ。",
        "強みは、打点ルートを見つけた時の押し切り力。七対子、ホンイツ、リーチ高打点のルートを選べるので、細かい和了で刻むより一度の大物手で着順を変える麻雀になっている。平均和了点はリーグ上位で、噛み合う半荘では一撃で着順を変えられる。放銃の質スコア-2.91は極端に悪いわけではなく、全てが無謀な押しというより、テンパイまで遅れて押し返す局が増え、その結果として苦しい放銃が混ざっている印象。個性そのものを消す必要はない。",
        "改善点は、勝負手ではない局の撤退速度と、重い手を追う基準。リーチの非推奨率25.69%が高めで、和了率が低いわりに放銃率17.23%が重い。打点種が薄い配牌、親の仕掛けが速い局、南場で2着を守れば十分な局では、満貫ルートを無理に追わず速度か守備に寄せたい。副露の質スコア-0.38も低く、鳴いた後の形・打点が足りない仕掛けが混ざっている。放銃率を2ポイント下げつつ、先制テンパイ率を少し上げるのが一番効く。",
    ),
}
