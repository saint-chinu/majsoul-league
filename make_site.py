...
# Comments are reviewed manually; routine data refreshes keep this dated assessment.
_REVIEWED_ANALYSIS = json.loads(
    (Path(__file__).resolve().parent / "data" / "player_analysis.json").read_text(encoding="utf-8")
)
MANUAL_NEW_LEAGUE_ANALYSIS = {
    player: tuple(sections) for player, sections in _REVIEWED_ANALYSIS["new"].items()
}
# Newcomers have no old-league games, so their reviewed assessment also applies to lifetime.
MANUAL_LIFETIME_ANALYSIS = {
    **MANUAL_NEW_LEAGUE_ANALYSIS,
    **{player: tuple(sections) for player, sections in _REVIEWED_ANALYSIS["lifetime"].items()},
}
ANALYSIS_REVIEW_NOTE = (
    f'{_REVIEWED_ANALYSIS["reviewed_on"]} 分析更新。{_REVIEWED_ANALYSIS["basis"]}'
)
...
def analysis_paragraphs(text: str) -> str:
    return "".join(f"<p>{esc(paragraph)}</p>" for paragraph in text.split("\n\n") if paragraph)
...
def render_player_panel(
    ...
    review_note = (
        f'<p class="subnote">{esc(ANALYSIS_REVIEW_NOTE)}</p>'
        if manual_analysis and player in manual_analysis else ""
    )
    ...
    <section class="tab-panel" id="panel-{esc(panel_key)}" data-panel="{esc(panel_key)}">
      ...
      <section class="analysis-grid">
        <article class="analysis-card">
          <h3>雀風と現在地</h3>
          {analysis_paragraphs(style)}
        </article>
        <article class="analysis-card">
          <h3>成長した点・強み</h3>
          {analysis_paragraphs(strength)}
        </article>
        <article class="analysis-card">
          <h3>強みを伸ばす次の一手</h3>
          {analysis_paragraphs(advice)}
        </article>
      </section>
    </section>
    """
...
def main() -> None:
    ...
    lifetime_player_panels = "\n".join(
        render_player_panel(
            ...
            title_label="通算",
            use_manual_analysis=False,
            manual_analysis=MANUAL_LIFETIME_ANALYSIS,
        )
        for player in lifetime_player_names
    )
    panels = stats_panels + "\n" + new_player_panels + "\n" + lifetime_player_panels
