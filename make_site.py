...
MANUAL_PLAYER_ANALYSIS: dict[str, tuple[str, str, str]] = {
    "流れ者金融": (
        "累計スコアと平均順位が抜けていて、単発の上振れではなく長期で勝つ形ができている。和了率、トップ率、トップキープ率が高く、さらに放銃平均打点が軽いので、押しているのに致命傷を受けにくい。副露率も高めで、門前の打点だけに寄らず、鳴きで局の速度を支配して相手の勝負手を間に合わせない場面が多いタイプ。三麻で強い「先に手を進め、危険になったら浅い傷で抜ける」が数字に出ている。",
        "一番大きい強みは、半荘全体の温度管理。浮いた後に雑に局へ参加せず、かといって守りすぎて追いつかれるわけでもない。必要な局では和了を取りに行き、不要な局では失点を薄くする判断ができている。役満回数もあり、守備的な優等生ではなく、相手の高打点レンジに正面から付き合える爆発力もある。リーグ内ではかなり完成度の高い万能型寄りの攻撃型で、負け筋を自分で増やしにくいのが強い。",
        "改善点は、独走時の押し返しをさらに冷たくすること。現状でも十分勝っているが、強者ゆえに押せてしまう局があり、2着以下が勝負手で来た時に受ける必要のない直撃リスクが残る。トップ目や大きく浮いた半荘では、加点の価値より局消化の価値が上がるので、終盤の一向聴押しや愚形テンパイ押しを少しだけ削ると、平均順位はさらに安定する。",
    ),
    "ひなんじょ": (
        "立直率が最も高く、門前リーチを中心に局面へ圧をかけるタイプ。ツモ率も高いので、リーチ後に他家の対応を待つだけでなく、自分で決着まで持ち込めている。累計スコアと平均順位も上位で、攻撃選択がきちんと収支に変換されている。一方で放銃平均打点が重めなので、攻め負けた時の削られ方が大きく、勝つ半荘と沈む半荘の落差が出やすい。",
        "強みは、門前で手を作って相手に守備判断を強制できること。副露で細かく拾うより、リーチの打点とツモ抽選で局面を一気に取るスタイルが合っている。シーズン4と7でMVPを取っているように、リーチの入り方と場況が噛み合うと一気にシーズンを持っていける爆発力がある。勝負手の押し切り性能は高く、周りから見ると「先制された時に一番面倒」な部類。",
        "改善点は、押し返された時の撤退ラインをもう少し明確にすること。放銃率そのものより、刺さった時の打点が痛い。親の高打点気配、ドラポン、終盤リーチに対して、自分の待ちの強さ、残り巡目、現物枚数をもう一段厳しく見ると安定する。攻撃力はすでに十分なので、数局だけ降りる選択を増やしても魅力は消えない。むしろ大沈みを削ることで、MVP級シーズン以外の底上げができる。",
    ),
    "鯛ofカルピス": (
        "ツモ率がトップで、勝負手を自力決着に持ち込む力がかなり強い。和了率も上位なので、手が入った時にテンパイ止まりで終わらず、きちんと点棒化できている。攻めの質は高く、他家の守備を突破しなくてもツモで点差を広げられるのが特徴。ただし放銃率と放銃平均打点が重く、攻めた局の反動も大きい。好調時は一気に伸びるが、噛み合わない半荘では失点が連鎖しやすい。",
        "強みは、ツモ決着で盤面を壊せること。ロン頼みではないので、相手が守っても点棒を持ってこられる。シーズン2、3、5でチーム優勝に絡んでいて、チーム戦では大きいプラスを作る役割を持てる。押す局の火力と決着力はリーグ内でも上位で、親番や高打点手を持った時の脅威度はかなり高い。",
        "改善点は、攻撃失敗時の損切り。自分の手が十分に見える局でも、相手の打点が濃い時は放銃平均を抑える意識がほしい。特に終盤の一向聴押し、親相手の無筋連打、ドラ絡み副露への押し返しは、あがり率より失点期待値を優先したい。ツモ力と和了力は落とさず、危険局だけ降りる精度を上げると、プラスの山を残したままマイナスの谷を浅くできる。",
    ),
    "アリスkey": (
        "序盤シーズンはやや苦しんでいたが、後半にかけて内容が明確に上向いている。シーズン6以降は大きく崩れにくくなり、シーズン7ではプラス、シーズン8も微マイナスで踏みとどまっている。累計だけ見るとまだ苦しいが、最近の内容は初期とはかなり違う。副露率は高めで、局を進める判断は積極的。以前よりも無理押しが減り、速度参加と撤退の整理が進んでいるように見える。",
        "強みは、回を重ねるごとに押し引きと参加判断が整ってきている点。平均放銃点が極端に悪くなく、和了率も中位を保っているので、土台は改善している。シーズン4と2でチーム優勝経験があり、チーム戦の中で役割を果たせる力もある。今は「負け方がマシになってきた」段階で、ここからトップ率を上げられると累計評価が一気に変わる。伸びしろ込みで一番コメントが更新されやすいプレイヤー。",
        "改善点は、トップを取り切る決定力。後半シーズンで安定はしてきたので、次は2着止まりや微マイナスをトップに変える局面作りが課題。副露で速度を取る局と、門前で打点を見る局の線引きをもう少し濃くしたい。特に南場の2着目、僅差トップ目では、安い仕掛けで局を進めるのか、リーチ含みで着順を決めにいくのかを早めに決めるとよい。改善傾向はかなり良いので、雑に攻撃量を増やすより、勝負局の選定を磨きたい。",
    ),
    "29ちゃん": (
        "放銃率が最も低く、放銃平均打点も軽い。守備面の安定感がはっきり出ているタイプ。三麻は一度の放銃で半荘が壊れやすいので、この低放銃はかなり価値が高い。役満回数も多く、守っているだけではなく、チャンス手では大きく仕留める力もある。一方で和了率と立直率は低めで、自分から局を取りに行く回数は少ない。負けない力は高いが、勝ち切るための能動性にはまだ伸びしろがある。",
        "強みは、余計な失点をしないこと。相手の高打点気配に付き合わず、半荘を壊さない能力が高い。シーズン6ではチーム優勝もあり、守備寄りの安定感がチーム成績に貢献している。さらに役満を引ける爆発力があるので、耐えて大物手で返す展開も持っている。守備型にありがちな決定力不足だけで終わっていないのが面白い。",
        "改善点は、和了率を少しだけ上げること。守備を崩す必要はないが、序盤の良形固定、役牌バック、北抜き後の速度判断で拾える和了を増やしたい。特に自分が親の局、ラス目の南場、相手がまだ遅い序盤は、守備の貯金を使ってもう少し前に出てもいい。低放銃という土台があるので、攻撃参加を少し増やしても致命傷にはなりにくい。小さい和了を増やせると平均順位が一段上がる。",
    ),
    "葡萄海ぶどう": (
        "平均和了点がトップで、打点を作る力が一番強く出ている。立直率も高めで、門前高打点や勝負手の破壊力があるタイプ。高い手を仕上げた時の圧はかなり強く、相手からすると安易に前へ出にくい存在。一方で累計スコアと平均順位は苦しく、あがれなかった局の失点や着順落ちが響いている。つまり「高い手をあがる力」はあるが、「その高打点局まで半荘を壊さず運ぶ力」がまだ成績に直結しきっていない。",
        "強みは、1回の和了で局面をひっくり返せること。安い手を細かく刻むより、打点を見て一撃で順位を変える麻雀になっている。平均和了点トップはかなり明確な個性で、ここは無理に丸める必要がない。むしろこの武器があるから、相手のリーチや副露に対しても「押せば見返りがある」局を作れる。ただし毎局それをやると放銃と着順落ちにつながるので、武器の出しどころが重要。",
        "改善点は、打点志向と守備の切り替えを早くすること。高打点を狙う価値はあるが、和了率、放銃率、トップキープ率が下位なので、勝負手ではない局の撤退を増やしたい。特に序盤で打点の種が薄い局、他家の親番でドラが見えている局、南場で着順が守れる局は、無理に高打点ルートへ寄せず速度か守備へ倒す方がよい。平均和了点の強みは残したまま、参加しない局を少し増やせると、マイナス半荘の深さがかなり変わる。",
    ),
}
